...
def callback_more(topic:str,msg:str):
    ...
    if topic == f'cmnd/{C.CLASS_ID}/{C.DEVICE}/LED_1':
        led_1.set_on_off_str(msg)
        mqtt.publish(f'stat/{C.CLASS_ID}/{C.DEVICE}/LED_1',led_1.on_off()) 

    elif topic == f'cmnd/{C.CLASS_ID}/{C.DEVICE}/THRESHOLD':
        gas_threshold = int(msg)
        print(f'new {{변수}}: {gas_threshold}')  
        mqtt.publish(f'stat/{C.CLASS_ID}/{C.DEVICE}/THRESHOLD',str(gas_threshold))
...
def pir_on():
    mqtt.publish(f'tele/{C.CLASS_ID}/{C.DEVICE}/PIR','on',retain=True)
    print('PIR on')

def pir_off():
    mqtt.publish(f'tele/{C.CLASS_ID}/{C.DEVICE}/PIR','off',retain=True)
    print('PIR off')
...
"""
NEW: Function to calibrate gas sensor
"""
def calibrate_gas (samples=20):
    total =9
    print("Calibrating gas sensor..")
    for i in range (Samples):
        Value = gas.read()
        total += value
        print("sample", 1 +1, value)
        time.sleep(0.2)
        baseline = total // samples
        print("Gas baseline:", baseline)
        return baseline
...
def mqtt_on_connect():
    ...
    mqtt.publish(f'tele/{C.CLASS_ID}{C.DEVICE}/INFO', msg, retain=True)
    on_connect_more()   
...
def mqtt_callback(in_topic,in_msg):
    ...
    if topic == f'cmnd/{C.CLASS_ID}{C.DEVICE}/TELEPERIOD':
        TELEPERIOD = int(msg)
        timer_sensor.set(period=TELEPERIOD*1000)
        tele_msg = {'TELEPERIOD': msg}
        mqtt.publish(f'stat/{C.CLASS_ID}{C.DEVICE}/TELEPERIOD', msg, retain=True)
        mqtt.publish(f'tele/{C.CLASS_ID}{C.DEVICE}/INFO', tele_msg, retain=True )
...
def send_data():
    ...
    mqtt.publish(f'tele/{C.CLASS_ID}{C.DEVICE}/SENSOR',json.dumps(data))
